#!/usr/bin/env python3
"""
Cron Job Setup Script for Plants Data Collector

This script sets up a cron job to run the plants_data_collector.py script every 5 minutes between 6 AM and 8 PM.
"""
...
def setup_cron_job():
    """Set up a cron job to run plants_data_collector.py every 5 minutes between 6 AM and 8 PM"""
    
    # Get the absolute path to the plants_data_collector.py script
    script_dir = Path(__file__).resolve().parent
    collector_script = script_dir / "plants_data_collector.py"
...
def remove_cron_job():
    """Remove the cron job for plants_data_collector.py"""
    try:
        # Get current crontab content
        current_crontab = subprocess.check_output(["crontab", "-l"], stderr=subprocess.DEVNULL).decode('utf-8')
    except subprocess.CalledProcessError:
    ...
    # Remove our job from the crontab
    cron_line = f"*/5 6-20 * * * {collector_script}"
    new_crontab = "\n".join(line for line in current_crontab.splitlines() if cron_line not in line)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Set up or manage cron job for plants data collector')
